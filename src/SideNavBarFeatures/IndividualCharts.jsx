...
export default function IndividualCharts({ individualData = [] }) {
  ...
  if (!individualData || individualData.length === 0) {
    return (
      <div className="mt-2 rounded-lg flex flex-col items-center justify-center w-full h-96 bg-slate-50 dark:bg-slate-900 shadow-lg border border-slate-200 dark:border-slate-800 p-6">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 mb-4">
          Individual Expense Breakdown
        </h2>
        <p className="text-slate-500 dark:text-slate-400">
          No individual expense data available
        </p>
      </div>
    );
  }
  ...
  const categories = Array.from(
    new Set(
      individualData.flatMap((person) =>
        Object.keys(person).filter((key) => key !== "name")
      )
    )
  );
  ...
  return (
    <div className="flex gap-10">
      <div className="mt-2 rounded-lg  gap-1 items-center justify-center  bg-white dark:bg-slate-900 shadow-xl border border-slate-200 dark:border-slate-800 p-4">
        ...
        >
          <ResponsiveBar
            ...
            barAriaLabel={(e) =>
              `${e.id}: ${e.formattedValue} for ${e.indexValue}`
            }
          />
        </div>
        <div className=" pt-2 flex flex-wrap justify-center gap-4">
          {categories.map((category, index) => (
            <div
              key={category}
              className="flex items-center gap-3 px-5 py-2 bg-white dark:bg-gray-800 rounded-full shadow-md border border-gray-200 dark:border-gray-700"
            >
              <div
      ...
      <div className="mt-2 rounded-lg  bg-white dark:bg-slate-900 shadow-xl border border-slate-200 dark:border-slate-800 p-6">
        {individualData.length > 0 && (
          <div className="flex flex-col gap-6">
            {/* Highest Spender */}
            <div className="flex items-center  gap-4 justify-between bg-slate-50 dark:bg-slate-800 rounded-lg px-6 py-4 shadow-sm">
              ...
              <div className="text-lg font-bold text-slate-800 dark:text-slate-100">
                {individualData
                  .sort((a, b) => {
                    const aSum = Object.entries(a).reduce(
                      (sum, [key, val]) => (key !== "name" ? sum + val : sum),
                      0
                    );
                    const bSum = Object.entries(b).reduce(
                      (sum, [key, val]) => (key !== "name" ? sum + val : sum),
                      0
                    );
                    return bSum - aSum;
                  })[0]
                  ?.name.toUpperCase() || "N/A"}
              </div>
            </div>
              ...
              <div className="text-lg font-bold text-slate-800 dark:text-slate-100">
                {(() => {
                  const categoryTotals = categories.map((cat) => ({
                    category: cat,
                    sum: individualData.reduce(
                      (sum, person) => sum + (person[cat] || 0),
                      0
                    ),
                  }));
                  return (
                    categoryTotals.sort((a, b) => b.sum - a.sum)[0]?.category ||
                    "N/A"
                  );
                })()}
              </div>
